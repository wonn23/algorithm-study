...
def stair(data):
    ...
    if n == 3:
        ...
        return data[2] + max(data[0],data[1])
    ...
    Table[2] = max(data[2] + data[0],data[2] + data[1])
    
    for i in range(3, n):
        # i 번째 계단에서의 점수를 저장해 주세요.
        Table[i] = max(data[i]+Table[i-2],data[i]+data[i-1]+Table[i-3]) 
...
def eating(data):
    ...
    # 초깃값은 모두 0으로 초기화 해요.
    Table = [[0 for i in range(3)] for j in range(n)] # DP의 데이터 리스트 초기화
    ...
    for i in range(1, n):
        ...
        Table[i][0] = max(Table[i-1][1],Table[i-1][2]) + data[i][0] # 마지막 날의 음식 선호도 값은 전날 먹은 짬뽕, 볶음밥 최대 선호도와 오늘의 짜장 선호도를 합한 값이다.
        Table[i][1] = max(Table[i-1][0],Table[i-1][2]) + data[i][1]
        Table[i][2] = max(Table[i-1][0],Table[i-1][1]) + data[i][2]
    
    # 음식 선호도의 최댓값을 반환해 주세요.
    return max(Table[n-1][0],Table[n-1][1],Table[n-1][2]) # 오늘의 최대 선호도는 Table 3개 중에서 가장 큰 값이다.
...
def lining(n):
    ...
    Table = [0]*(n+1) # 첫 사람이 0명일 때도 생각해줘야하므로 리스트를 n+1개 초기화 해줘야한다.
...
def makeNumber(n, m):
    ...
    for i in range(1, n+1):
        for j in range(1, m+1):
            if i >= j:
                Table[i] = (Table[i] + Table[i-j])%1000000007
