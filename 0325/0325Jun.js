...
const slowCounter = () => {
  ...
  const p = parseInt(counter.innerText);
  message.innerHTML += `${curClick}. 현재 값은... ${p}<br>`;

  window.setTimeout(() => {
    const sum = parseInt(counter.innerText) + 1;
    message.innerHTML += `${curClick}. 더한 값은... ${sum}<br>`;
